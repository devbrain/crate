...
void arc_lzw_decompressor::init_state() {
    state_ = squashed_ ? state::READ_FIRST_CODE : state::SKIP_HEADER;
    bit_buffer_ = 0;
    bits_left_ = 0;
    init_dictionary();
    code_ = 0;
...
void arc_lzw_decompressor::init_dictionary() {
...
bool arc_lzw_decompressor::try_read_code(const byte*& ptr, const byte* end, int& code) {
...
result_t<stream_result> arc_lzw_decompressor::decompress_some(
    byte_span input,
    ...
    while (state_ != state::DONE) {
        ...
        switch (state_) {
            case state::SKIP_HEADER: {
                // For crunched modes, skip the first header byte
                if (in_ptr >= in_end) {
                    if (input_finished) {
                        state_ = state::DONE;
